...
void main() {
  group('UnifiedLogger File Capture', () {
    ...
    test('should capture ALL categories to file even when category filtering is enabled', () async {
      // Given: Category filtering is enabled (only system and auth)
      // This is the default production configuration
      expect(UnifiedLogger.enabledCategories, equals({LogCategory.system, LogCategory.auth}));
      ...
      Log.info('Relay log', category: LogCategory.relay); // NOT enabled for console
      Log.info('Video log', category: LogCategory.video); // NOT enabled for console
      Log.info('UI log', category: LogCategory.ui); // NOT enabled for console
      Log.info('API log', category: LogCategory.api); // NOT enabled for console
      Log.info('Storage log', category: LogCategory.storage); // NOT enabled for console
      ...
      expect(newLogsCount, equals(7),
        reason: 'ALL 7 logs should be captured to file, even those from disabled categories');
    ...
    test('should capture ALL log levels to file even when level filtering is enabled', () async {
      ...
      Log.verbose('Verbose log', category: LogCategory.system); // Below threshold
      ...
      expect(newLogsCount, equals(5),
        reason: 'ALL 5 logs should be captured to file, even verbose and debug');
    ...
    test('should capture logs from disabled categories for bug report export', () async {
      ...
      expect(newLogsCount, equals(7),
        reason: 'User log export must include ALL categories for debugging remote issues');

      // Verify critical relay logs are included
      final newLogs = exportedLogs.skip(beforeCount).toList();
      final relayLogs = newLogs.where((log) => log.category == LogCategory.relay);
      expect(relayLogs.length, equals(2),
        reason: 'Relay connection errors are critical for debugging and must be captured');
    });
  });
}
